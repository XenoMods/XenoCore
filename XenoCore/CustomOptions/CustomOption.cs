...
namespace XenoCore.CustomOptions {
	...
	public abstract partial class CustomOption {
		...
		/// </summary>
		public Dictionary<string, Func<string>> LocalizationArguments;
		...
		public virtual bool ValueShown => true;
		...
		private void OnGameOptionCreated(OptionBehaviour o) {
			...
			try {
				...
				XenoPlugin.Logger.LogWarning($"Exception in OnGameOptionCreated for option \"{Name}\" ({Type}): {e}");
			}
		...
		private static string GetCallingPluginId() {
			...
			for (var i = 3; i < stackTrace.GetFrames().Length; i++) {
		...
		public static TitleOption AddTitle(string Title) {
			return new TitleOption(Title);
		}
		...
		public static CustomNumberOption AddNumber(string id, string name, bool saveValue, float value,
			float min = 0.25F, float max = 5F, float increment = 0.25F) {
		...
		public static CustomNumberOption AddNumber(string id, string name, float value, float min = 0.25F,
			float max = 5F, float increment = 0.25F) {
		...
		/// <param name="max">The highest value permitted, may be overriden if <paramref name="value"/> is higher</param>
		/// <param name="increment">The increment or decrement steps when <see cref="CustomNumberOption.Increase"/> or <see cref="CustomNumberOption.Decrease"/> are called</param>
		public static CustomNumberOption AddNumber(string name, bool saveValue, float value, float min = 0.25F,
			float max = 5F, float increment = 0.25F) {
		...
		/// <param name="max">The highest value permitted, may be overriden if <paramref name="value"/> is higher</param>
		/// <param name="increment">The increment or decrement steps when <see cref="CustomNumberOption.Increase"/> or <see cref="CustomNumberOption.Decrease"/> are called</param>
		public static CustomNumberOption AddNumber(string name, float value, float min = 0.25F, float max = 5F,
		...
		private protected void SetValue(object value, bool raiseEvents) {
			...
			if (raiseEvents && OnValueChanged != null && AmongUsClient.Instance && PlayerControl.LocalPlayer &&
			    AmongUsClient.Instance.AmHost) {
				var lastValue = value;

				var args = OnValueChangedEventArgs(value, Value);
			...
			try {
				...
				XenoPlugin.Logger.LogWarning($"Failed to update game setting value for option \"{Name}\": {e}");
			}
	...
	public class CustomToggleOption : CustomOption {
		...
		/// <param name="value">The default value</param>
		protected internal CustomToggleOption(string id, string name, bool saveValue, bool value) : base(id, name,
			saveValue, CustomOptionType.Toggle, value) {
			ValueChanged += (sender, args) => {
				if (GameSetting is ToggleOption && AmongUsClient.Instance && PlayerControl.LocalPlayer &&
				    AmongUsClient.Instance.AmHost && ConfigEntry != null) ConfigEntry.Value = (bool) Value;
			};

			ConfigEntry = saveValue ? XenoPlugin.Instance.Config.Bind(PluginID, ConfigID, (bool) DefaultValue) : null;
			SetValue(ConfigEntry?.Value ?? GetDefaultValue(), false);

			StringFormat = (sender, value) => (bool) value
				? XenoLang.ON.Get()
				: XenoLang.OFF.Get();
		}

		private protected override OptionOnValueChangedEventArgs
			OnValueChangedEventArgs(object value, object oldValue) {
	...
	/// </summary>
	public class CustomNumberOption : CustomOption, INumberProvider {
		/// <summary>
		/// The config entry used to store this option's value.
		/// </summary>
		/// <remarks>
		...
		protected internal CustomNumberOption(string id, string name, bool saveValue, float value, float min = 0.25F,
			float max = 5F, float increment = 0.25F) : base(id, name, saveValue, CustomOptionType.Number, value) {
			...
			ValueChanged += (sender, args) => {
				if (GameSetting is NumberOption && AmongUsClient.Instance && PlayerControl.LocalPlayer &&
				    AmongUsClient.Instance.AmHost && ConfigEntry != null) ConfigEntry.Value = (float) Value;
			};

			ConfigEntry = saveValue ? XenoPlugin.Instance.Config.Bind(PluginID, ConfigID, (float) DefaultValue) : null;
			SetValue(ConfigEntry?.Value ?? GetDefaultValue(), false);
	...
	public class CustomStringOption : CustomOption {
		...
		public CustomStringOption(string id, string name, bool saveValue, string[] values) : base(id, name, saveValue,
			CustomOptionType.String, 0) {
			Values = values;

			ValueChanged += (sender, args) => {
				if (GameSetting is StringOption && AmongUsClient.Instance && PlayerControl.LocalPlayer &&
				    AmongUsClient.Instance.AmHost && ConfigEntry != null) ConfigEntry.Value = (int) Value;
			};

			ConfigEntry = saveValue ? XenoPlugin.Instance.Config.Bind(PluginID, ConfigID, (int) DefaultValue) : null;
			RememberedValue = ConfigEntry?.Value ?? GetDefaultValue();
			SetValue(ConfigEntry?.Value ?? GetDefaultValue(), false);
		...
		public void SetValues(string[] Values) {
			this.Values = Values;
			SetValue(RememberedValue, false);
		}
		...
		public override string ToString() {
			return LanguageManager.Get(base.ToString());
		}
	}

	public class TitleOption : CustomOption {
		public override bool ValueShown => false;

		public TitleOption(string Title) : base("title", Title, false, CustomOptionType.Title, false) {
		}

		private protected override void GameOptionCreated(OptionBehaviour o) {
			if (!(o is ToggleOption Title)) return;
